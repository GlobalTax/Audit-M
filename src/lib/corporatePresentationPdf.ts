...
// Professional audit color palette (matches admin theme)
const COLORS = {
  navy: '#1B1F3D',
  accent: '#4F6AE8',
  darkGray: '#333333',
...
// Format date with Catalan support
...
export const generateCorporatePresentationPDF = (data: PresentationData): jsPDF => {
  ...
  const addHeader = () => {
    ...
    doc.text(brandName.toUpperCase(), margin, isLandscape ? 12 : 16);
  ...
  const addFooter = () => {
    ...
    doc.text(`${pageNum} / ${totalPages}`, pageWidth - margin, pageHeight - 8, {
      align: 'right',
    });
  };
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(content.coverSubtitle, margin, pageHeight * 0.5);
  ...
  pageNum++;
  doc.text(`${pageNum} / ${totalPages}`, pageWidth - margin, pageHeight - 20, {
    align: 'right',
  });
  ...
  if (data.sections.aboutUs) {
    ...
    currentY = addBodyText(
      content.aboutText,
      currentY,
      isLandscape ? contentWidth * 0.65 : contentWidth
    );
  ...
  if (data.sections.keyFigures) {
    ...
    figures.forEach((fig, index) => {
      ...
      // Value
      const accentText = hexToRgb(COLORS.accent);
      doc.setTextColor(accentText.r, accentText.g, accentText.b);
      doc.setFontSize(isLandscape ? 22 : 26);
      doc.setFont('helvetica', 'bold');
      doc.text(fig.value, x + figWidth / 2, y + (isLandscape ? 18 : 22), {
        align: 'center',
      });
      ...
      doc.setFontSize(isLandscape ? 9 : 10);
      doc.setFont('helvetica', 'normal');
      doc.text(fig.label, x + figWidth / 2, y + (isLandscape ? 28 : 34), {
        align: 'center',
      });
    });
  ...
  // Service Areas
  if (data.sections.serviceAreas) {
    doc.addPage();
    addHeader();
    currentY = isLandscape ? 30 : 40;

    currentY = addSectionTitle(content.serviceAreasTitle, currentY);
    currentY += 5;
  ...
  if (data.sections.team) {
    ...
    members.forEach((member, index) => {
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.text(member.name, x + memberWidth / 2, y + (isLandscape ? 28 : 32), {
        align: 'center',
      });
      ...
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      doc.text(member.role, x + memberWidth / 2, y + (isLandscape ? 34 : 38), {
        align: 'center',
      });
    });
  ...
  if (data.sections.contact) {
    ...
    // Contact box
    const boxHeight = 50;
    const navyColor = hexToRgb(COLORS.navy);
    ...
    doc.setTextColor(accentText.r, accentText.g, accentText.b);
    doc.setFont('helvetica', 'bold');
    doc.text(content.contactInfo.web, pageWidth - margin - 15, contactY + 10, {
      align: 'right',
    });
...
export const downloadCorporatePresentationPDF = (data: PresentationData) => {
  ...
  const fileName = `${brandName}_Presentation_${recipient.replace(/\s+/g, '_')}_${formatSuffix}.pdf`;
  doc.save(fileName);
};
