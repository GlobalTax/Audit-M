...
const BlogDetail = () => {
  ...
  const { data: previewData, isLoading: isPreviewLoading, error: previewError } = usePreviewContent(
    "blog_post", articleId || "", previewToken && articleId ? previewToken : null
  );
  ...
  const { data: dbData, isLoading: isDbLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await supabase.from("blog_posts").select("*")
        .or(`slug_es.eq.${slug},slug_en.eq.${slug}`).eq("status", "published").single();
  ...
  useEffect(() => {
    if (dbData?.id && !previewToken) { incrementViewMutation.mutate(dbData.id); }
    ...
    return (
      <div className="container mx-auto px-4 py-16">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <Button asChild><Link to="/blog"><ArrowLeft className="mr-2 h-4 w-4" />{t('blog.backToBlog')}</Link></Button>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="bg-muted/30 border-b border-border/50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <Breadcrumb><BreadcrumbList><BreadcrumbItem><BreadcrumbLink asChild><Link to="/">{t('breadcrumb.home')}</Link></BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbLink asChild><Link to="/blog">{t('nav.blog')}</Link></BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage>{post?.title}</BreadcrumbPage></BreadcrumbItem></BreadcrumbList></Breadcrumb>
        </div>
      </div>
