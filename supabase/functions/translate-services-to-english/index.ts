...
serve(async (req) => {
  ...
  try {
    console.log('🚀 Starting services translation to English...');
    ...
    const { data: services, error: servicesError } = await supabase
      .from('services')
      .select('*')
      .or('name_en.is.null,slug_en.is.null,metodologia_en.is.null,servicios_transversales_en.is.null');
    ...
    for (const service of services || []) {
      try {
        ...
        if (service.name_en && service.slug_en && service.description_en && 
            service.metodologia_en && service.servicios_transversales_en) {
        ...
        if (translatedEn.metodologia) {
          updateData.metodologia_en = translatedEn.metodologia;
        }

        // Add translated servicios_transversales if exists
        if (translatedEn.servicios_transversales) {
          updateData.servicios_transversales_en = translatedEn.servicios_transversales;
        }

        // Add translated stats if exists
        if (translatedEn.stats) {
          updateData.stats_en = translatedEn.stats;
        }
    ...
    console.log('🎉 Services translation to English complete!');
